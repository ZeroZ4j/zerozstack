...
In a multi-tenant product the manifest is not one document: the name, icons and colour belong to the
...
The cache name carries the build version — `zeroz4j-shell-0.6.0` — so a deployment evicts the
